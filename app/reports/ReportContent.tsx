 ...
 * category dashboard, findings table, comparison-with-a-previous-audit —
 * extracted out of app/reports/[id]/page.tsx so it can be reused: as that
 * standalone page (heading + "back to reports" link included), and embedded
 * directly inside a Theme's own page (heading omitted — the theme's name
 * is already showing above it there).
 *
 * A caller that switches `auditRunId` between renders (e.g. Theme Detail's
 * Report section, choosing a different past run from Previous Audits)
 * should render this with `key={auditRunId}` so React remounts it fresh
...
export function ReportContent({
  ...
  const comparableRuns = useMemo(() => {
    ...
    return siblingRuns.filter((r) => r._id !== auditRunId && r.status === "complete" && r.themeId?._id === themeId);
  }, [siblingRuns, auditRun, auditRunId]);
  ...
  function runComparison(newBaselineId: string) {
    ...
    setDiffLoading(true);
    fetch(`/api/reports/${auditRunId}/diff?baseline=${newBaselineId}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  if (notFound || !auditRun) return <p className="text-sm text-zinc-500">Audit run not found.</p>;

  return (
    <div className="flex flex-col gap-8">
      {showHeading && <h2 className="text-xl font-semibold text-zinc-950 dark:text-zinc-50">{auditRun.themeId?.name ?? "Audit report"}</h2>}
          ...
          {comparableRuns.length > 0 && (
            <div className="flex flex-col gap-4 border-t border-black/[.08] pt-8 dark:border-white/[.145]">
              ...
              {diff && (
                <>
                  <a
                    href={`/api/reports/${auditRunId}/diff/export?baseline=${baselineId}&format=csv`}
