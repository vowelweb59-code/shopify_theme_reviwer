...
function readGoogleBannerFromLocation(): { kind: "connected" | "error"; message?: string } | null {
  ...
  if (google === "error") return { kind: "error", message: params.get("googleError") ?? undefined };
  return null;
}
...
function GoogleSheetsPanel() {
  ...
  // Read once, synchronously, from the initial location — no flash of a
  // missing banner while waiting for an effect to run after mount.
  const [banner] = useState(readGoogleBannerFromLocation);
...
export function SettingsContent() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h2 className="text-xl font-semibold text-zinc-950 dark:text-zinc-50">General</h2>
      ...
      {!config ? (
        ...
        <div className="flex flex-col gap-6 rounded-lg border border-black/[.08] p-5 dark:border-white/[.145]">
          <div>
            <h3 className="text-sm font-medium text-zinc-950 dark:text-zinc-50">Blocker severities</h3>
            <p className="mt-1 text-xs text-zinc-500">
          ...
          <div>
            <h3 className="text-sm font-medium text-zinc-950 dark:text-zinc-50">Minimum coverage for a READY claim</h3>
            <p className="mt-1 text-xs text-zinc-500">
